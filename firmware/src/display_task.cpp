...
void DisplayTask::run() {
    ...
    if (spr_.createSprite(TFT_AWIDTH, TFT_AHEIGHT) == nullptr) {
    ...
    const int RADIUS = min(TFT_AHEIGHT, TFT_AWIDTH) / 2;
    ...
    while(1) {
        ...
        if (!sk_demo_mode) {
          if (num_positions > 1) {
            int32_t height = (state.current_position - state.config.min_position) * TFT_AHEIGHT / (state.config.max_position - state.config.min_position);
            spr_.fillRect(0, TFT_AHEIGHT - height, TFT_AWIDTH, height, FILL_COLOR);
          }

          spr_.setFreeFont(&Roboto_Light_60);
          spr_.drawNumber(state.current_position, TFT_AWIDTH / 2, TFT_AHEIGHT / 2 - VALUE_OFFSET, 1);
          spr_.setFreeFont(&DESCRIPTION_FONT);
          int32_t line_y = TFT_AHEIGHT / 2 + DESCRIPTION_Y_OFFSET;
          ...
          while (start < end) {
            ...
            spr_.drawString(String(buf), TFT_AWIDTH / 2, line_y, 1);
            start = newline + 1;
            line_y += spr_.fontHeight(1);
          }

          if (num_positions > 0) {
            spr_.drawLine(TFT_AWIDTH/2 + RADIUS * cosf(left_bound), TFT_AHEIGHT/2 - RADIUS * sinf(left_bound), TFT_AWIDTH/2 + (RADIUS - 10) * cosf(left_bound), TFT_AHEIGHT/2 - (RADIUS - 10) * sinf(left_bound), TFT_WHITE);
            spr_.drawLine(TFT_AWIDTH/2 + RADIUS * cosf(right_bound), TFT_AHEIGHT/2 - RADIUS * sinf(right_bound), TFT_AWIDTH/2 + (RADIUS - 10) * cosf(right_bound), TFT_AHEIGHT/2 - (RADIUS - 10) * sinf(right_bound), TFT_WHITE);
          }
          if (DRAW_ARC) {
            spr_.drawCircle(TFT_AWIDTH/2, TFT_AHEIGHT/2, RADIUS, TFT_DARKGREY);
          }
          ...
          if (num_positions > 0 && ((state.current_position == state.config.min_position && state.sub_position_unit < 0) || (state.current_position == state.config.max_position && state.sub_position_unit > 0))) {
            spr_.fillCircle(TFT_AWIDTH/2 + (RADIUS - 10) * cosf(raw_angle), TFT_AHEIGHT/2 - (RADIUS - 10) * sinf(raw_angle), 5, DOT_COLOR);
            if (raw_angle < adjusted_angle) {
              for (float r = raw_angle; r <= adjusted_angle; r += 2 * PI / 180) {
                spr_.fillCircle(TFT_AWIDTH/2 + (RADIUS - 10) * cosf(r), TFT_AHEIGHT/2 - (RADIUS - 10) * sinf(r), 2, DOT_COLOR);
              }
              spr_.fillCircle(TFT_AWIDTH/2 + (RADIUS - 10) * cosf(adjusted_angle), TFT_AHEIGHT/2 - (RADIUS - 10) * sinf(adjusted_angle), 2, DOT_COLOR);
            } else {
              for (float r = raw_angle; r >= adjusted_angle; r -= 2 * PI / 180) {
                spr_.fillCircle(TFT_AWIDTH/2 + (RADIUS - 10) * cosf(r), TFT_AHEIGHT/2 - (RADIUS - 10) * sinf(r), 2, DOT_COLOR);
              }
            ...
            spr_.fillCircle(TFT_AWIDTH/2 + (RADIUS - 10) * cosf(adjusted_angle), TFT_AHEIGHT/2 - (RADIUS - 10) * sinf(adjusted_angle), 5, DOT_COLOR);
          }
        } else {
          if (strncmp(state.config.text, "SKDEMO_Scroll", 13) == 0) {
            spr_.fillRect(0, 0, TFT_AWIDTH, TFT_AHEIGHT, spr_.color565(150, 0, 0));
            spr_.setFreeFont(&Roboto_Thin_24);
            spr_.drawString("Scroll", TFT_AWIDTH / 2, TFT_AHEIGHT / 2, 1);
            bool detent = false;
            ...
            spr_.fillCircle(TFT_AWIDTH/2 + (RADIUS - 16) * cosf(adjusted_angle), TFT_AHEIGHT/2 - (RADIUS - 16) * sinf(adjusted_angle), detent ? 8 : 5, TFT_WHITE);
          } else if (strncmp(state.config.text, "SKDEMO_Frames", 13) == 0) {
            int32_t width = (state.current_position - state.config.min_position) * TFT_AWIDTH / (state.config.max_position - state.config.min_position);
            spr_.fillRect(0, 0, width, TFT_AHEIGHT, spr_.color565(0, 150, 0));
            spr_.setFreeFont(&Roboto_Light_60);
            spr_.drawNumber(state.current_position, TFT_AWIDTH / 2, TFT_AHEIGHT / 2, 1);
            spr_.setFreeFont(&Roboto_Thin_24);
            spr_.drawString("Frame", TFT_AWIDTH / 2, TFT_AHEIGHT / 2 - DESCRIPTION_Y_OFFSET - VALUE_OFFSET, 1);
          } else if (strncmp(state.config.text, "SKDEMO_Speed", 12) == 0) {
            spr_.fillRect(0, 0, TFT_AWIDTH, TFT_AHEIGHT, spr_.color565(0, 0, 150));
            ...
            spr_.setFreeFont(&Roboto_Thin_24);
            if (roundedSpeed == 0) {
              spr_.drawString("Paused", TFT_AWIDTH / 2, TFT_AHEIGHT / 2 + DESCRIPTION_Y_OFFSET + VALUE_OFFSET, 1);

              spr_.fillRect(TFT_AWIDTH / 2 + 5, TFT_AHEIGHT / 2 - 20, 10, 40, TFT_WHITE);
              spr_.fillRect(TFT_AWIDTH / 2 - 5 - 10, TFT_AHEIGHT / 2 - 20, 10, 40, TFT_WHITE);
            } else {
              char buf[10];
              snprintf(buf, sizeof(buf), "%0.1fx", roundedSpeed);
              spr_.drawString(buf, TFT_AWIDTH / 2, TFT_AHEIGHT / 2 + DESCRIPTION_Y_OFFSET + VALUE_OFFSET, 1);

              uint16_t x = TFT_AWIDTH / 2;
              for (uint8_t i = 0; i < max(1, abs(state.current_position)); i++) {
                drawPlayButton(spr_, x, TFT_AHEIGHT / 2, sgn(roundedSpeed) * 20, 40, TFT_WHITE);
